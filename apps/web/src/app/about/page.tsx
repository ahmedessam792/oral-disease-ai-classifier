  ...
  return (
    <div className="mx-auto max-w-[1320px] px-4 pb-20 pt-10 sm:px-6 sm:pt-14">
      <header className="max-w-[58ch]">
        <h1
          tabIndex={-1}
          className="text-h1 font-semibold text-ink outline-none sm:text-display"
        >
          About Arcus
        </h1>
        <p className="mt-5 text-lead text-ink-soft">
          Arcus takes a photograph of teeth, gums, or oral tissue, runs it through
          a trained ResNet50V2, and reports what it predicted, how sure it was, and
          every probability it assigned. Its output is never a diagnosis.
        </p>
      </header>

      {/* How the analysis works */}
      <section aria-labelledby="how-heading" className="mt-16">
        <h2 id="how-heading" className="text-h2 font-semibold text-ink">
        ...
        <ol className="mt-8 grid gap-x-6 gap-y-8 sm:grid-cols-2 lg:grid-cols-5">
          {STEPS.map(({ icon: Icon, title, body }, index) => (
            <li key={title}>
              ...
              <p className="mt-1.5 text-ui text-ink-soft">{body}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section aria-labelledby="model-heading" className="mt-16">
        <h2 id="model-heading" className="text-h2 font-semibold text-ink">
          The model
        </h2>
        <p className="mt-3 max-w-[58ch] text-body text-ink-soft">
          ...
          pixels. Its head is a softmax over the six classes below — the distribution
          you see in a result is the model&apos;s own output, not a derived score.
        </p>
        <ModelClasses />
      </section>
      ...
      <section aria-labelledby="arch-heading" className="mt-16">
        <h2 id="arch-heading" className="text-h2 font-semibold text-ink">
          Architecture
        </h2>
      ...
      <section aria-labelledby="limits-heading" className="mt-16">
        <h2 id="limits-heading" className="text-h2 font-semibold text-ink">
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          ...
          >
            <ul className="flex flex-col gap-4">
              {LIMITATIONS.map((item) => (
                <li key={item.title}>
                  <h3 className="text-ui font-medium text-ink">{item.title}</h3>
                  <p className="mt-1 max-w-[58ch] text-ui text-ink-soft">
                    {item.body}
                  </p>
                </li>
              ))}
          ...
          >
            <div className="flex max-w-[58ch] flex-col gap-3 text-ui text-ink-soft">
      ...
      <Link
        href="/"
        className="mt-14 inline-flex h-12 items-center gap-2 rounded-md bg-teal px-6 text-ui font-medium text-white transition-colors hover:bg-teal-deep"
      >
        Analyze an image
